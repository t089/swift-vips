...
extension VIPSTests {
    ...
    struct ConvolutionGeneratedTests {
        ...
        func testSharpenOperations() throws {
            ...
            let original =
                try VIPSImage.gaussnoise(width: 100, height: 100, sigma: 10.0, mean: 128.0)
        ...
        func testCustomKernelTypes() throws {
            ...
            let identityKernel = try identity.linear(0.0, 0.0)
                .insert(sub: VIPSImage.black(width: 1, height: 1).linear(0.0, 1.0), x: 1, y: 1)
            ...
            sobelXData = try sobelXData.insert(
                sub: VIPSImage.black(width: 1, height: 1).linear(0.0, -1.0),
                x: 0,
                y: 0
            )
            sobelXData = try sobelXData.insert(
            ...
            sobelXData = try sobelXData.insert(
                sub: VIPSImage.black(width: 1, height: 1).linear(0.0, -2.0),
                x: 0,
                y: 1
            )
            sobelXData = try sobelXData.insert(
        ...
        func testSeparableConvolutionVsRegular() throws {
            ...
            for i in 0..<size {
                ...
                gaussianKernel = try gaussianKernel.insert(
                    sub: VIPSImage.black(width: 1, height: 1).linear(0.0, value),
                    x: 0,
                    y: i
                )
                sum += value
        ...
        func testBlurParameterEffects() throws {
            let noisyImage = try VIPSImage.gaussnoise(
                width: 100,
                height: 100,
                sigma: 50.0,
                mean: 128.0

...
struct ResampleTests {
    ...
    func testResizeInterpolation() throws {
        for format in nonComplexFormats {
            let testImage = try makeTestImage(width: 100, height: 100, value: 128.0).cast(format)
    ...
    func testThumbnailBufferWithOptions() throws {
        ...
        let thumb = try VIPSImage.thumbnail(buffer: .init(data), width: 150, height: 100, 
                                           crop: .centre, intent: .relative)
